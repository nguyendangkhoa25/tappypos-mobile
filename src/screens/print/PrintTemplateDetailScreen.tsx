  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      >
        ...
        <Text className="text-lg font-bold text-gray-900 dark:text-white flex-1">{name || '...'}</Text>
        <TouchableOpacity
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 16 }}>
          ...
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-4">
            <Text className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{t('printTemplates.detail.nameLabel')}</Text>
            <TextInput
              value={name}
          ...
          {/* Toggle fields */}
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-4">
            <Text className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">{t('printTemplates.detail.contentLabel')}</Text>
            {CONFIG_FIELD_KEYS.map((field, idx) => (
              <View key={field.key}>
                {idx > 0 && <View className="h-px bg-gray-100 dark:bg-gray-700 my-2" />}
                <View className="flex-row items-center justify-between py-1">
                  <Text className="text-sm text-gray-800 dark:text-gray-200">{t(field.i18nKey)}</Text>
                  <Switch
          ...
          {/* Footer text */}
          <View className="bg-white dark:bg-gray-800 rounded-2xl p-4">
            <Text className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{t('printTemplates.detail.footerLabel')}</Text>
            <TextInput
          ...
          <View className="bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl p-4 flex-row items-center gap-3">
            ...
            <Text className="text-sm text-indigo-700 dark:text-indigo-400 flex-1">
