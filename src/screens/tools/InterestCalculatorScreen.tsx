...
export function InterestCalculatorScreen({ navigation }: Props) {
  ...
  return (
    <KeyboardAvoidingView className="flex-1 bg-gray-50" behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View className="bg-indigo-600 px-6 pb-5" style={{ paddingTop: top + 16 }}>
        <TouchableOpacity onPress={() => navigation.goBack()} className="mb-3">
          <MaterialCommunityIcons name="arrow-left" size={24} color="white" />
        </TouchableOpacity>
        <Text className="text-2xl font-bold text-white">{t('interestCalc.title')}</Text>
      </View>
      ...
      <ScrollView className="flex-1 px-4 pt-4" keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 40 }}>
        <View className="flex-row items-start bg-indigo-50 rounded-xl px-3 py-2.5 border border-indigo-100 mb-3">
          ...
          <Text className="text-xs text-indigo-600 leading-4 flex-1">{t('interestCalc.hint')}</Text>
        </View>
        ...
        <View className="flex-row items-start bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 mb-3">
          ...
          <Text className="flex-1 text-xs text-amber-700 leading-4">{t('interestCalc.disclaimer')}</Text>
        </View>

        <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100">
          <Text className="text-sm font-semibold text-gray-700 mb-2">{t('interestCalc.principal')}</Text>
        ...
        <View className="flex-row mb-3" style={{ gap: 10 }}>
          <View className="flex-1 bg-white rounded-2xl p-4 border border-gray-100">
            <Text className="text-sm font-semibold text-gray-700 mb-2">{t('interestCalc.rate')}</Text>
            <TextInput
          ...
          <View className="flex-1 bg-white rounded-2xl p-4 border border-gray-100">
            <Text className="text-sm font-semibold text-gray-700 mb-2">{t('interestCalc.term')}</Text>
            <TextInput
              className="border border-gray-300 rounded-xl px-3 py-3 text-base bg-white text-gray-900"
              value={term}
        ...
        <View className="bg-white rounded-2xl p-4 mb-3 border border-gray-100">
          <Text className="text-sm font-semibold text-gray-700 mb-3">{t('interestCalc.interestType')}</Text>
          <View className="flex-row rounded-xl overflow-hidden border border-gray-200">
            {(['simple', 'compound'] as InterestType[]).map((type) => (
              ...
              >
                <Text className={`text-sm font-semibold ${interestType === type ? 'text-white' : 'text-gray-600'}`}>
                  {t(`interestCalc.${type}`)}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        {results && (
          <>
            <View className="bg-indigo-50 rounded-2xl p-4 border border-indigo-100 mb-3">
              <Text className="text-sm font-bold text-indigo-700 mb-3">{t('interestCalc.resultsTitle')}</Text>
              <ResultRow
            ...
            {results.compoundPeriods && results.compoundPeriods.length > 0 && (
              <View className="bg-white rounded-2xl p-4 border border-gray-100">
                <Text className="text-sm font-bold text-gray-700 mb-3">{t('interestCalc.breakdown')}</Text>
                <View className="flex-row mb-2">
                  <Text className="w-12 text-xs font-semibold text-gray-400">{t('interestCalc.period')}</Text>
                  <Text className="flex-1 text-xs font-semibold text-gray-400 text-right">{t('interestCalc.earned')}</Text>
                  <Text className="flex-1 text-xs font-semibold text-gray-400 text-right">{t('interestCalc.balance')}</Text>
                </View>
...
function ResultRow({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  ...
  return (
    <View className="flex-row justify-between items-center mb-2">
      <Text className={`text-sm ${highlight ? 'font-bold text-indigo-800' : 'text-gray-600'}`}>{label}</Text>
      <Text className={`text-sm ${highlight ? 'font-bold text-indigo-700' : 'font-semibold text-gray-800'}`}>
...
function CompoundPeriodRow({ period }: { period: CompoundPeriod }) {
  ...
  return (
    <View className="flex-row py-1.5 border-t border-gray-100">
      <Text className="w-12 text-xs font-semibold text-indigo-600">{period.label}</Text>
      <Text className="flex-1 text-xs text-emerald-600 text-right">+{fmt(Math.round(period.earned))}</Text>
      <Text className="flex-1 text-xs font-semibold text-gray-700 text-right">{fmt(Math.round(period.balance))}</Text>
    </View>
  );
}
