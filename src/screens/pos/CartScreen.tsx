...
export function CartScreen({ navigation }: POSScreenProps<'Cart'>) {
  ...
  const { items, updateQty, removeItem, discount, getTotal, clearCart } = useCartStore();
    ...
    <View className="flex-1 bg-white" style={{ paddingTop: insets.top }}>
      ...
      {items.length === 0 ? (
        ...
        <>
            ...
            renderItem={({ item }) => (
              <View className="flex-row items-center px-4 py-4 border-b border-gray-50">
                <View className="flex-1">
                  ...
                  <Text className="text-sm text-primary font-bold mt-0.5">
          ...
          {/* Summary + checkout */}
          <View
            className="bg-white border-t border-gray-100 px-4 pt-4 pb-4"
