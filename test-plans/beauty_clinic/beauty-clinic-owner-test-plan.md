# E2E Test Plan — Beauty Clinic · SHOP_OWNER

> **Shop type:** BEAUTY_CLINIC (Thẩm mỹ viện)  
> **Role:** SHOP_OWNER — all 23 features  
> **Credentials:** phone `0911000099` · password `Clinic@2026` · shop `bcln-99989`  
...
**Default roles:** SHOP_OWNER · MANAGER · TECHNICIAN (aesthetician/specialist) · RECEPTIONIST · CASHIER
...
| PRE-02 | Beauty Clinic tenant is ACTIVE | Confirmed in master dashboard | — | |
| PRE-03 | At least 2 treatments/services seeded | e.g. Chăm sóc da, Laser | — | |
| PRE-04 | At least 1 non-owner staff member (TECHNICIAN) exists | For appointment assignment | — | |
...
| A-04 | Enter valid Beauty Clinic tenant ID → submit | → LoginScreen; shop name "Thẩm mỹ viện …" shown read-only | — | |
...
| N-07 | No PAWN / gold tiles | "Cầm đồ", "Giá vàng" absent | — | Beauty Clinic has no PAWN |
| N-08 | No VENDOR / purchase-order tiles | "Nhà cung cấp", "Nhập hàng" absent | — | |
| N-09 | Shop name in top bar | Displays clinic name, not "Quản lý cửa hàng" | — | |
...
## Section 4 — POS / Bán hàng (BarberServiceScreen — Beauty Clinic)

> **Note:** Beauty Clinic loads `BarberServiceScreen` — same as Nail Shop. Quick phrases are beauty-specific: **Chăm sóc da · Laser · HIFU · Triệt lông**.

### 4.1 Opening the POS

| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| P-01 | Tap "Bán hàng" tab | BarberServiceScreen loads; treatment/service grid shown | — | |
...
| P-05 | Search by treatment name | Grid filters within 300 ms | — | |
| P-06 | Clear search | Full grid restored | — | |
| P-07 | No match | EmptyState shown | — | |

### 4.2 Adding treatments to cart

| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| P-08 | Tap a treatment | Service detail bottom sheet slides up | — | |
| P-09 | Quick phrase bar shows beauty phrases | "Chăm sóc da", "Laser", "HIFU", "Triệt lông" chips shown | — | Beauty Clinic specific |
| P-10 | Tap a quick phrase | Phrase appended to note field | — | |
| P-11 | Assign a specialist (TECHNICIAN) to the treatment | Specialist avatar highlighted | — | |
...
| P-15 | Add second treatment (different specialist) | Second line in cart | — | |
| P-16 | Cart badge shows total + specialist count | Correct | — | |

### 4.3 Cart review

| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| P-17 | Tap cart icon | Cart items list; treatment name, specialist, note, price, qty per row | — | |
...
| P-27 | Preferences banner | Shows skin type / allergies / preferred treatments if set | — | Beauty Clinic context |
...
| P-34 | Bank account picker shows saved accounts | Clinic's bank accounts listed | — | |
...
| P-52 | Switch back to treatment grid | Grid restored; cart preserved | — | |
...
| OR-11 | Treatment name, specialist, note, qty, price per line | All fields shown | — | |
| OR-12 | Payment method and amount shown | Correct | — | |
| OR-13 | Assigned specialist shown | Staff name visible | — | |
...
## Section 6 — Appointments (APPOINTMENT — Beauty Clinic critical)
...
| AP-28 | Search for a treatment | Name + duration shown | — | |
| AP-29 | Select a treatment | Row added; total duration auto-increases | — | |
| AP-30 | Add 2 treatments | Both rows; duration = sum | — | |
| AP-31 | Tap specialist dropdown on treatment row | Employee picker; "— Chưa gán —" at top | — | |
| AP-32 | Select a specialist | Name shown on row | — | |
| AP-33 | Select "— Chưa gán —" | Specialist removed | — | |
| AP-34 | Tap "×" on treatment row | Removed; duration reduced | — | |
...
| AP-42 | Treatments: name + specialist + price per row | All rows | — | |
| AP-43 | Total estimate at bottom | Sum of treatment prices | — | |
...
| AP-65 | Add/remove treatment → save | Treatments updated | — | |
| AP-66 | Change assigned specialist → save | New specialist reflected | — | |
...
| SQ-02 | Staff cards with avatar, name, treatments | Renders correctly | — | |
...
| CU-11 | Fill skin type / allergies / preferred treatments | Saved; banner shown at POS checkout | — | Beauty Clinic: skin type instead of hair type |
...
| CU-18 | Edit skin preferences → save | Saved; banner shown at POS | — | |
...
| PR-11 | Select a **treatment/service** type | Unit auto-set; `durationMinutes` field appears | — | |
| PR-12 | Fill name + price + unit + category + duration → save | Treatment created with duration | — | |
| PR-13 | Select a **retail** product type (e.g. skincare product) | Standard fields; no duration | — | |
...
| ST-11 | Select role TECHNICIAN | Chip highlighted | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| ST-22 | Open StaffPerformanceScreen | Cards with metrics per specialist | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| SE-01 | Open Settings | SettingsScreen; clinic name + avatar at top | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| SE-10 | Tap "Thông tin cửa hàng" | Name, address, phone, type "Thẩm mỹ viện" | — | |
| SE-11 | Edit clinic name → save | Updated | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| PF-01 | Open ProfileScreen | Name, phone, role SHOP_OWNER, clinic name | — | |
...
- [ ] **POS golden path (Beauty Clinic):** BarberServiceScreen → add treatment + assign specialist → select customer → cash checkout → OrderSuccess → back with empty cart
- [ ] **POS with QR:** treatment → bank transfer → VietQR generated
- [ ] **Quick phrases:** "Chăm sóc da", "Laser", "HIFU", "Triệt lông" chips shown in service bottom sheet
- [ ] **Loyalty at checkout:** customer with points → enable redemption → discount applied to total
- [ ] **Appointment golden path:** create (linked customer + 2 treatments + specialist) → confirm → check-in → POS redirect
- [ ] **ORDER_VIEW_ALL:** order list shows orders from other staff
- [ ] **Walk-in customer:** attempt delete "Khách lẻ" → blocked
- [ ] **Customer preferences:** save skin type/allergies → preferences banner shown at POS
- [ ] **Staff:** create TECHNICIAN with auto-password → credential modal shown
- [ ] **Product:** create treatment type with duration → in appointment service picker
