# E2E Test Plan — Lash & PMU Studio · SHOP_OWNER

> **Shop type:** LASH_PMU_STUDIO (Tiệm mi / Xăm thẩm mỹ)  
> **Role:** SHOP_OWNER — all 23 features  
> **Credentials:** phone `0911000077` · password `LashPMU@2026` · shop `lash-95672`  
...
**Default roles:** SHOP_OWNER · MANAGER · TECHNICIAN (lash artist/PMU artist) · RECEPTIONIST · CASHIER
...
| PRE-02 | Lash PMU Studio tenant is ACTIVE | Confirmed in master dashboard | — | |
| PRE-03 | At least 2 services seeded | e.g. Nối mi đơn Classic, Xăm mày Ombre | — | |
| PRE-04 | At least 1 non-owner staff member (TECHNICIAN/artist) exists | For appointment assignment | — | |
...
| A-04 | Enter valid Lash PMU tenant ID → submit | → LoginScreen; shop name "Tiệm Mi Xăm …" shown read-only | — | |
...
| N-07 | No PAWN / gold tiles | "Cầm đồ", "Giá vàng" absent | — | Lash PMU has no PAWN |
| N-08 | No VENDOR / purchase-order tiles | "Nhà cung cấp", "Nhập hàng" absent | — | |
| N-09 | Shop name in top bar | Displays studio name, not "Quản lý cửa hàng" | — | |
...
## Section 4 — POS / Bán hàng (BarberServiceScreen — Lash PMU Studio)

> **Note:** Lash PMU Studio loads `BarberServiceScreen` — same as Nail Shop. Quick phrases are lash/PMU-specific: **Nối mi · Xăm mày · Xăm môi · Xăm mí**.

### 4.1 Opening the POS

| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| P-01 | Tap "Bán hàng" tab | BarberServiceScreen loads; lash/PMU service grid shown | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| P-08 | Tap a lash/PMU service | Service detail bottom sheet slides up | — | |
| P-09 | Quick phrase bar shows lash/PMU phrases | "Nối mi", "Xăm mày", "Xăm môi", "Xăm mí" chips shown | — | Lash PMU specific |
| P-10 | Tap a quick phrase | Phrase appended to note field | — | |
| P-11 | Assign an artist (TECHNICIAN) to the service | Artist avatar highlighted | — | |
...
| P-15 | Add second service (different artist) | Second line in cart | — | |
| P-16 | Cart badge shows total + artist count | Correct | — | |

### 4.3 Cart review

| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| P-17 | Tap cart icon | Cart items list; service name, artist, note, price, qty per row | — | |
...
| P-27 | Preferences banner | Shows preferred lash style / glue allergy / notes if set | — | Lash PMU context |
...
| P-34 | Bank account picker shows saved accounts | Studio's bank accounts listed | — | |
...
| OR-11 | Service name, artist, note, qty, price per line | All fields shown | — | |
| OR-12 | Payment method and amount shown | Correct | — | |
| OR-13 | Assigned artist shown | Staff name visible | — | |
...
## Section 6 — Appointments (APPOINTMENT — Lash PMU Studio critical)
...
| AP-28 | Search for a lash/PMU service | Name + duration shown | — | |
| AP-29 | Select a service | Row added; total duration auto-increases | — | |
| AP-30 | Add 2 services | Both rows; duration = sum | — | |
| AP-31 | Tap artist dropdown on service row | Employee picker; "— Chưa gán —" at top | — | |
| AP-32 | Select an artist | Name shown on row | — | |
| AP-33 | Select "— Chưa gán —" | Artist removed | — | |
...
| AP-42 | Services: name + artist + price per row | All rows | — | |
...
| AP-66 | Change assigned artist → save | New artist reflected | — | |
...
| CU-11 | Fill preferred lash style / glue allergy / notes | Saved; banner shown at POS checkout | — | Lash PMU: lash/PMU-specific preferences |
...
| CU-18 | Edit lash/PMU preferences → save | Saved; banner shown at POS | — | |
...
| PR-11 | Select a **lash/PMU service** type (SERVICE) | Unit auto-set; `durationMinutes` field appears | — | |
| PR-12 | Fill name + price + unit + category + duration → save | Service created with duration | — | |
| PR-13 | Select a **retail** product type (e.g. BEAUTY/aftercare product) | Standard fields; no duration | — | |
...
| ST-11 | Select role TECHNICIAN (lash/PMU artist) | Chip highlighted | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| ST-22 | Open StaffPerformanceScreen | Cards with metrics per artist | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| SE-01 | Open Settings | SettingsScreen; studio name + avatar at top | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| SE-10 | Tap "Thông tin cửa hàng" | Name, address, phone, type "Tiệm mi / Xăm thẩm mỹ" | — | |
| SE-11 | Edit studio name → save | Updated | — | |
...
| ID | Step | Expected | Status | Notes |
|----|------|----------|--------|-------|
| PF-01 | Open ProfileScreen | Name, phone, role SHOP_OWNER, studio name | — | |
...
- [ ] **POS golden path (Lash PMU):** BarberServiceScreen → add lash/PMU service + assign artist → select customer → cash checkout → OrderSuccess → back with empty cart
- [ ] **POS with QR:** service → bank transfer → VietQR generated
- [ ] **Quick phrases:** "Nối mi", "Xăm mày", "Xăm môi", "Xăm mí" chips shown in service bottom sheet
- [ ] **Loyalty at checkout:** customer with points → enable redemption → discount applied to total
- [ ] **Appointment golden path:** create (linked customer + 2 services + artist) → confirm → check-in → POS redirect
- [ ] **ORDER_VIEW_ALL:** order list shows orders from other staff
- [ ] **Walk-in customer:** attempt delete "Khách lẻ" → blocked
- [ ] **Customer preferences:** save lash style/glue allergy → preferences banner shown at POS
- [ ] **Staff:** create TECHNICIAN (lash/PMU artist) with auto-password → credential modal shown
- [ ] **Product:** create service type with duration → in appointment service picker
